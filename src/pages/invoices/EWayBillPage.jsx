      ...
      <div className="eway-editor-scroll no-print">
        <section className="eway-pdf-editor">
          <div className="eway-editor-paper-head">
            <div>
              <h2>e-Way Bill</h2>
              <span>PDF-style E-Way Bill editor</span>
            </div>
            <div className="eway-editor-qr">
              <strong>QR</strong>
              <span>Generated after NIC API</span>
            </div>
          </div>

          <section className="eway-editor-section">
            <div className="eway-editor-section-title">1. E-WAY BILL Details</div>

            <div className="eway-editor-detail-row four">
              <label>
                <span>eWay Bill No.</span>
                <input value={form.ewayBillNo || ''} readOnly placeholder="Pending NIC generation"/>
              </label>
              <label>
                <span>Generated Date</span>
                <input value={form.generatedAt || ''} readOnly placeholder="After NIC generation"/>
              </label>
              <label>
                <span>Generated By</span>
                <input value={form.generatedBy || ''} readOnly placeholder="After NIC generation"/>
              </label>
              <label>
                <span>Valid Upto</span>
                <input value={form.validUntil || ''} readOnly placeholder="After NIC generation"/>
              </label>
            </div>

            <div className="eway-editor-detail-row six">
              <label>
                <span>Mode</span>
              ...
              <label>
                ...
                <div className="eway-inline-input">
              ...
              <label className="wide-doc">
                <span>Document Details</span>
                <select value={form.documentType} onChange={(e) => update('documentType', e.target.value)}>
                  <option>Tax Invoice</option><option>Bill of Supply</option><option>Delivery Challan</option><option>Bill of Entry</option><option>Others</option>
                </select>
                <div className="eway-editor-doc-pair">
              ...
              <label>
                ...
                <select value={form.transactionType} onChange={(e) => update('transactionType', e.target.value)}>
                  <option>Regular</option><option>Bill To - Ship To</option><option>Bill From - Dispatch From</option><option>Combination</option>
                </select>
              </label>
          ...
          <section className="eway-editor-section">
            <div className="eway-editor-section-title">2. Address Details</div>
            <div className="eway-editor-address-grid">
              <div className="eway-editor-address-box">
                <h4>From</h4>
                <label><span>GSTIN</span><input value={form.from?.gstin || ''} onChange={(e) => updateGroup('from', 'gstin', e.target.value.toUpperCase())} placeholder="GSTIN / URP"/></label>
                <label><span>Trade Name</span><input value={form.from?.tradeName || ''} onChange={(e) => updateGroup('from', 'tradeName', e.target.value)} placeholder="Company / workshop"/></label>
                <label><span>:: Dispatch From ::</span><textarea value={form.from?.address || ''} onChange={(e) => updateGroup('from', 'address', e.target.value)} placeholder="Address"/></label>
                <div className="eway-editor-address-bottom">
                  <input value={form.from?.place || ''} onChange={(e) => updateGroup('from', 'place', e.target.value)} placeholder="Place"/>
                  <input inputMode="numeric" maxLength={6} value={form.from?.pincode || ''} onChange={(e) => updateGroup('from', 'pincode', e.target.value.replace(/\D/g, '').slice(0,6))} placeholder="PIN"/>
                  <input inputMode="numeric" maxLength={2} value={form.from?.stateCode || ''} onChange={(e) => updateGroup('from', 'stateCode', e.target.value.replace(/\D/g, '').slice(0,2))} placeholder="State code"/>
                </div>
              </div>

              <div className="eway-editor-address-box">
                <h4>To</h4>
                <label><span>GSTIN</span><input value={form.to?.gstin || ''} onChange={(e) => updateGroup('to', 'gstin', e.target.value.toUpperCase())} placeholder="GSTIN / URP"/></label>
                <label><span>Trade Name</span><input value={form.to?.tradeName || ''} onChange={(e) => updateGroup('to', 'tradeName', e.target.value)} placeholder="Customer / destination"/></label>
                <label><span>:: Ship To ::</span><textarea value={form.to?.address || ''} onChange={(e) => updateGroup('to', 'address', e.target.value)} placeholder="Address"/></label>
                <div className="eway-editor-address-bottom">
                  ...
                  <input inputMode="numeric" maxLength={2} value={form.to?.stateCode || ''} onChange={(e) => updateGroup('to', 'stateCode', e.target.value.replace(/\D/g, '').slice(0,2))} placeholder="State code"/>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="eway-editor-section">
            <div className="eway-editor-section-title eway-editor-title-action">
              <span>3. Goods Details</span>
              <button className="eway-editor-add" type="button" onClick={() => update('items', [...form.items, newItem()])}>
                <Plus size={13}/>Add Goods
              </button>
            </div>

            <div className="eway-editor-goods">
              <div className="head">
                <span>HSN Code</span>
              ...
              {form.items.map((item, index) => (
                <div className="row" key={item.id}>
                  ...
                  <div className="eway-editor-product">
                  ...
                  <div className="eway-editor-qty">
                    <input inputMode="decimal" value={item.qty} onChange={(e) => updateItem(item.id, 'qty', dec(e.target.value))}/>
                  ...
                  <div className="eway-editor-tax-rate">
                    <input aria-label="CGST rate" inputMode="decimal" value={item.cgstRate} onChange={(e) => updateItem(item.id, 'cgstRate', dec(e.target.value))} placeholder="C"/>
                    <span>+</span>
                    <input aria-label="SGST rate" inputMode="decimal" value={item.sgstRate} onChange={(e) => updateItem(item.id, 'sgstRate', dec(e.target.value))} placeholder="S"/>
                    <span>+</span>
                    <input aria-label="IGST rate" inputMode="decimal" value={item.igstRate} onChange={(e) => updateItem(item.id, 'igstRate', dec(e.target.value))} placeholder="I"/>
                    <span>+</span>
                  ...
                  <button type="button" className="eway-editor-remove" aria-label="Remove goods item" onClick={() => update('items', form.items.filter((x) => x.id !== item.id))}>
                    <Trash2 size={14}/>
                  </button>
                </div>
              ))}
            ...
            <div className="eway-editor-tax-strip">
              <label><span>Tot. Tax'ble Amt</span><strong>{money.format(totals.taxableValue)}</strong></label>
              <label><span>CGST Amt</span><strong>{money.format(totals.cgst)}</strong></label>
              <label><span>SGST Amt</span><strong>{money.format(totals.sgst)}</strong></label>
              <label><span>IGST Amt</span><strong>{money.format(totals.igst)}</strong></label>
              <label><span>CESS Amt</span><strong>{money.format(totals.cess)}</strong></label>
              <label><span>CESS Non.Advol Amt</span><input inputMode="decimal" value={form.cessNonAdvolAmount || ''} onChange={(e) => update('cessNonAdvolAmount', dec(e.target.value))} placeholder="0.00"/></label>
              <label><span>Other Amt</span><input inputMode="decimal" value={form.otherAmount || ''} onChange={(e) => update('otherAmount', dec(e.target.value))} placeholder="0.00"/></label>
              <label><span>Total Inv.Amt</span><strong>{money.format(totals.totalValue)}</strong></label>
            </div>
          </section>

          <section className="eway-editor-section">
            <div className="eway-editor-section-title">4. Transportation Details</div>
            <div className="eway-editor-transport-row">
              <label>
                <span>Transporter ID & Name</span>
                <div className="eway-editor-doc-pair">
                  <input value={form.transport.transporterId} onChange={(e) => updateGroup('transport', 'transporterId', e.target.value.toUpperCase())} placeholder="Transporter ID / GSTIN"/>
          ...
          <section className="eway-editor-section">
            <div className="eway-editor-section-title">5. Vehicle Details</div>
            <div className="eway-editor-vehicle-table">
              <div className="head">
                <span>Mode</span>
              ...
              <div className="row">
                ...
                <input value={form.transport.vehicleNo} onChange={(e) => updateGroup('transport', 'vehicleNo', e.target.value.toUpperCase())} placeholder="Vehicle No."/>
                <input value={form.enteredFrom || ''} onChange={(e) => update('enteredFrom', e.target.value.toUpperCase())} placeholder="From"/>
                <input value={form.generatedAt || ''} readOnly placeholder="After NIC generation"/>
                ...
                <input value={form.multiVehicleInfo || ''} onChange={(e) => update('multiVehicleInfo', e.target.value)} placeholder="-"/>
                <input value={form.portal || '1'} onChange={(e) => update('portal', e.target.value)} placeholder="1"/>
              </div>
            </div>
          </section>

          <div className="eway-editor-notes">
            <label>
